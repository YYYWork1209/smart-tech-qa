...
public class ChatHistory implements ChatHistoryRepository{
    ...
    // TODO 这里是先暂时用Map来进行存储，后续再选择是存储到哪种类型数据库中,同时查询会话历史要结合用户数据
    ...
    public void saveChatHistory(String type,String chatId) {
        ...
        // 如果不存在，把当前type以及聊天记录id列表存入map集合（后续改为数据库）
        // 如果存在，返回当前type对应的id值列表（一种类型会有多个会话，同时还需对应用户id）
        // 返回值为当前类型下的所有会话id值
        List<String> chatIds = chatTypeAndId.computeIfAbsent(type, k -> new ArrayList<String>());
    ...
    public List<String> getChatHistory(String type) {
        ...
        return chatTypeAndId.getOrDefault(type,List.of());
